...
def decode(bytes, password):
    gen = itertools.cycle(password)
    result = []
    for byte in bytes:
        key = ord(next(gen))
...
with open('resources/problem59_cipher.txt') as f:
    contents = f.read()
contents = [int(x) for x in contents.split(',')]

all_passwords = itertools.product(string.ascii_lowercase, repeat=3)
illegal = '@#$%^&*\\/}{+'
...
for password in all_passwords:
    ...
    if all((s not in illegal for s in decoded)):
        print(decoded)
